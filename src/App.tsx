...
type TimelineRecord = { id: string; date: string; type: string; title: string; detail: string; source: 'system' | 'expense' | 'note' | 'mortality'; headsLost?: number }
...
function BatchesPage({ batches, expenses, batchNotes, mortalityRecords, selectedBatch, onAddBatch, onEditBatch, onOpenBatch, onEditNote, onDeleteNote, onDeleteMortality }: { batches: Batch[]; expenses: Expense[]; batchNotes: BatchNote[]; mortalityRecords: MortalityRecord[]; selectedBatch: Batch | null; onAddBatch: () => void; onEditBatch: (batch: Batch) => void; onOpenBatch: (batch: Batch) => void; onEditNote: (note: BatchNote) => void; onDeleteNote: (noteId: string) => void; onDeleteMortality: (record: MortalityRecord) => void }) {
  ...
  return <><section className="batches-toolbar"><p className="section-kicker">Grow-out operations</p><div className="batch-toolbar-actions">{selectedBatch && <button className="secondary-action" type="button" onClick={() => onEditBatch(selectedBatch)} title={`Edit ${selectedBatch.id}`}>Edit selected batch</button>}<button className="primary-action" type="button" onClick={onAddBatch}>Add livestock batch</button></div></section>
    ...
    {visibleBatch && <BatchTimeline batch={visibleBatch} expenses={expenses} batchNotes={batchNotes} mortalityRecords={mortalityRecords} onEditNote={onEditNote} onDeleteNote={onDeleteNote} onDeleteMortality={onDeleteMortality} />}
...
function BatchTimeline({ batch, expenses, batchNotes, mortalityRecords, onEditNote, onDeleteNote, onDeleteMortality }: { batch: Batch; expenses: Expense[]; batchNotes: BatchNote[]; mortalityRecords: MortalityRecord[]; onEditNote: (note: BatchNote) => void; onDeleteNote: (noteId: string) => void; onDeleteMortality: (record: MortalityRecord) => void }) {
  ...
  return <article className="performance-timeline batch-record-timeline"><div className="panel-heading"><div><p className="section-kicker">Selected batch record</p><h2>{batch.id} dates and notes</h2></div><span className="list-note">{timeline.length} recorded events</span></div><TimelineList batchId={batch.id} batchNotes={batchNotes} timeline={timeline} onEditNote={onEditNote} onDeleteNote={onDeleteNote} onDeleteMortality={onDeleteMortality} /></article>
}

function TimelineList({ batchId, batchNotes, timeline, onEditNote, onDeleteNote, onDeleteMortality }: { batchId: string; batchNotes: BatchNote[]; timeline: TimelineRecord[]; onEditNote: (note: BatchNote) => void; onDeleteNote: (noteId: string) => void; onDeleteMortality: (record: MortalityRecord) => void }) {
  return <div className="timeline-list">{timeline.map((note) => <div className="timeline-item" key={`${note.source}-${note.id}`}><time>{formatDate(note.date)}</time><span className={`timeline-marker ${note.type.toLowerCase().replaceAll(' ', '-')}`} aria-label={note.type === 'Mortality' ? 'Mortality event' : note.type === 'Medicine' ? 'Medicine event' : undefined}>{note.type === 'Mortality' ? '!' : note.type === 'Medicine' ? '+' : ''}</span><div><i>{note.type}</i><b>{note.title}</b><p>{note.detail}</p></div><div className="timeline-actions">{note.source === 'note' ? <><button type="button" onClick={() => { const noteRecord = batchNotes.find((item) => item.id === note.id); if (noteRecord) onEditNote(noteRecord) }}>Edit note</button><button className="danger-action" type="button" onClick={() => onDeleteNote(note.id)}>Remove note</button></> : note.source === 'mortality' ? <button className="danger-action" type="button" onClick={() => onDeleteMortality({ id: note.id, batchId, date: note.date, headsLost: note.headsLost ?? 0, note: note.detail === 'No reason recorded' ? '' : note.detail })}>Remove mortality</button> : <span>View only</span>}</div></div>)}</div>
}
...
function PerformancePanel({ batches, expenses, batchNotes, mortalityRecords, performanceByBatch, saleEstimates, onAddWeight, onAddNote, onSaveEstimate, onEditNote, onDeleteNote, onDeleteMortality }: { batches: Batch[]; expenses: Expense[]; batchNotes: BatchNote[]; mortalityRecords: MortalityRecord[]; performanceByBatch: Record<string, PerformanceSample[]>; saleEstimates: Record<string, { price: number; weight: number }>; onAddWeight: () => void; onAddNote: () => void; onSaveEstimate: (batch: Batch, price: number, weight: number) => void; onEditNote: (note: BatchNote) => void; onDeleteNote: (noteId: string) => void; onDeleteMortality: (record: MortalityRecord) => void }) {
  const activeBatches = batches.filter((batch) => batch.status === 'Active')
  ...
  const weeklyNotes: TimelineRecord[] = samples.map((sample, index) => ({ id: `week-${index}`, date: chartDates[index], type: index === 0 ? 'Starting check' : 'Weekly check', title: `${sample.averageWeight} kg average weight`, detail: index === 0 ? 'Starting weight recorded on arrival' : `${sample.feedBags} of 50 kg feed bags used this week`, source: 'system' }))
  ...
  return <section className="performance-panel">
    ...
    <article className="performance-timeline"><div className="panel-heading"><div><p className="section-kicker">Batch record</p><h2>{selectedDate ? `Dates and notes · ${formatDate(selectedDate)}` : 'Dates and notes'}</h2></div>{selectedDate ? <button className="quiet-action" type="button" onClick={() => setSelectedDayIndex(null)}>Show all notes</button> : <span className="list-note">{timeline.length} recorded events</span>}</div><TimelineList batchId={selectedBatch.id} batchNotes={batchNotes} timeline={visibleTimeline} onEditNote={onEditNote} onDeleteNote={onDeleteNote} onDeleteMortality={onDeleteMortality} /></article>
  </section>
}
