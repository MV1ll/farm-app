...
function BatchTimeline({ batch, expenses, batchNotes, mortalityRecords, onEditNote, onDeleteNote, onDeleteMortality }: { batch: Batch; expenses: Expense[]; batchNotes: BatchNote[]; mortalityRecords: MortalityRecord[]; onEditNote: (note: BatchNote) => void; onDeleteNote: (noteId: string) => void; onDeleteMortality: (record: MortalityRecord) => void }) {
  ...
  return <article className="performance-timeline batch-record-timeline"><div className="panel-heading"><div><p className="section-kicker">Selected batch record</p><h2>{batch.id} dates and notes</h2></div><span className="list-note">{timeline.length} recorded events</span></div><div className="timeline-list">{timeline.map((note) => <div className="timeline-item" key={`${note.source}-${note.id}`}><time>{formatDate(note.date)}</time><span className={`timeline-marker ${note.type.toLowerCase().replaceAll(' ', '-')}`} aria-label={note.type === 'Mortality' ? 'Mortality event' : note.type === 'Medicine' ? 'Medicine event' : undefined}>{note.type === 'Mortality' ? '!' : note.type === 'Medicine' ? '+' : ''}</span><div><i>{note.type}</i><b>{note.title}</b><p>{note.detail}</p>{note.source === 'note' && <div className="timeline-actions"><button type="button" onClick={() => { const noteRecord = batchNotes.find((item) => item.id === note.id); if (noteRecord) onEditNote(noteRecord) }}>Edit</button><button className="danger-action" type="button" onClick={() => onDeleteNote(note.id)}>Remove</button></div>}{note.source === 'mortality' && <div className="timeline-actions"><button className="danger-action" type="button" onClick={() => onDeleteMortality({ id: note.id, batchId: batch.id, date: note.date, headsLost: note.headsLost ?? 0, note: note.detail === 'No reason recorded' ? '' : note.detail })}>Remove mortality</button></div>}</div></div>)}</div></article>
}
