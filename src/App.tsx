...
function App() {
  ...
  useEffect(() => {
    const loadFarmData = async () => {
      try {
        ...
        setFeedInventory(data.feedInventory.map((item) => ({ id: item.id, feedName: item.feed_name, species: item.species, stage: item.stage, bagsOnHand: Number(item.bags_on_hand), reorderLevelBags: Number(item.reorder_level_bags), bagWeightKg: Number(item.bag_weight_kg), unitCostPhp: Number(item.unit_cost_php), supplier: item.supplier_name ?? 'No supplier' })))
  ...
  const saveFeedInventory = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const saved = await saveToDatabase('saveFeedInventory', item)
      setFeedInventory((current) => [...current.filter((entry) => entry.feedName !== feedName), { id: saved.id, ...item }].sort((first, second) => first.feedName.localeCompare(second.feedName)))
  ...
  return <div className="farm-app">
    ...
    {showFeedForm && <FeedInventoryForm onClose={() => setShowFeedForm(false)} onSubmit={saveFeedInventory} />}
...
function FeedInventoryPage({ inventory, onAddFeed }: { inventory: FeedInventoryItem[]; onAddFeed: () => void }) {
  const totalBags = inventory.reduce((total, item) => total + item.bagsOnHand, 0)
  const totalValue = inventory.reduce((total, item) => total + item.bagsOnHand * item.unitCostPhp, 0)
  const lowStock = inventory.filter((item) => item.bagsOnHand <= item.reorderLevelBags)
  return <><section className="batches-toolbar"><div><p className="section-kicker">Feed store</p><h2>Current stock</h2></div><button className="primary-action" type="button" onClick={onAddFeed}>Add feed item</button></section>
    ...
    <article className="table-panel inventory-list"><div className="panel-heading"><div><p className="section-kicker">Live stock</p><h2>Feed inventory</h2></div><span className="list-note">Bag counts update from PostgreSQL</span></div>{inventory.length ? <div className="inventory-table" role="table" aria-label="Feed inventory"><div className="inventory-head" role="row"><span>Feed</span><span>Animal</span><span>Stage</span><span>On hand</span><span>Reorder at</span><span>Bag size</span><span>Unit cost</span><span>Supplier</span></div>{inventory.map((item) => <div className="inventory-row" role="row" key={item.id}><span><b>{item.feedName}</b><small className={item.bagsOnHand <= item.reorderLevelBags ? 'stock-state low' : 'stock-state'}>{item.bagsOnHand <= item.reorderLevelBags ? 'Reorder needed' : 'Stock available'}</small></span><span>{item.species}</span><span>{item.stage}</span><span><b>{item.bagsOnHand}</b> bags</span><span>{item.reorderLevelBags} bags</span><span>{item.bagWeightKg} kg</span><span>{php.format(item.unitCostPhp)}</span><span>{item.supplier}</span></div>)}</div> : <div className="empty-inventory"><b>No feed items yet.</b><span>Add your first starter, grower, or finisher feed item to begin tracking stock.</span></div>}</article></>
}
