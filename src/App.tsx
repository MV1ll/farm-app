...
type TimelineRecord = { id: string; date: string; type: string; title: string; detail: string; source: 'system' | 'performance' | 'expense' | 'note' | 'mortality'; headsLost?: number; averageWeight?: number; feedBags?: number; noteText?: string }
...
function App() {
    ...
    const loadFarmData = async () => {
      try {
        ...
        const performance = data.performance.reduce<Record<string, PerformanceSample[]>>((records, entry) => {
          ...
          records[batch.batch_code] = [...(records[batch.batch_code] ?? []), { id: entry.id, weekEnding: entry.week_ending.slice(0, 10), averageWeight: Number(entry.average_weight_kg), feedBags: Number(entry.feed_bags ?? 0), note: '' }].sort((first, second) => first.weekEnding.localeCompare(second.weekEnding))
          return records
        }, {})
  ...
  return <div className="farm-app">
    ...
    <main className="workspace">
      ...
      {section === 'Batches' ? <BatchesPage batches={batches} expenses={expenses} batchNotes={batchNotes} mortalityRecords={mortalityRecords} selectedBatch={selectedBatch} onAddBatch={() => setShowBatchForm(true)} onEditBatch={setEditingBatch} onOpenBatch={setSelectedBatch} onEditNote={setEditingNote} onDeleteNote={deleteBatchNote} onDeleteMortality={deleteMortalityRecord} onEditExpense={setEditingExpense} onDeleteExpense={deleteExpenseRecord} /> : section === 'Performance' ? <PerformancePanel batches={batches} expenses={expenses} batchNotes={batchNotes} mortalityRecords={mortalityRecords} performanceByBatch={performanceByBatch} saleEstimates={saleEstimates} onAddWeight={() => setActiveEntry('Weekly weight check')} onAddNote={() => setActiveEntry('Batch note')} onSaveEstimate={saveSaleEstimate} onEditNote={setEditingNote} onDeleteNote={deleteBatchNote} onDeleteMortality={deleteMortalityRecord} onEditPerformance={setEditingPerformance} onDeletePerformance={deletePerformanceRecord} onEditExpense={setEditingExpense} onDeleteExpense={deleteExpenseRecord} /> : section === 'Feed inventory' ? <FeedInventoryPage batches={batches} inventory={feedInventory} onAddFeed={() => setShowFeedForm(true)} /> : section === 'Expenses' ? <ExpensesPage expenses={expenses} onAddExpense={() => setShowExpenseForm(true)} /> : <Dashboard batches={batches} expenses={expenses} totalAnimals={totalAnimals} totalCost={totalCost} operationalCapital={operationalCapital} onAddCapital={() => setShowCapitalForm(true)} onRemoveCapital={() => setShowRemoveCapitalForm(true)} onEntry={(kind) => kind === 'Expense' ? setShowExpenseForm(true) : setActiveEntry(kind)} onBatches={() => setSection('Batches')} onOpenBatch={openBatch} />}
    </main>
...
function BatchesPage({ batches, expenses, batchNotes, mortalityRecords, selectedBatch, onAddBatch, onEditBatch, onOpenBatch, onEditNote, onDeleteNote, onDeleteMortality, onEditExpense, onDeleteExpense }: { batches: Batch[]; expenses: Expense[]; batchNotes: BatchNote[]; mortalityRecords: MortalityRecord[]; selectedBatch: Batch | null; onAddBatch: () => void; onEditBatch: (batch: Batch) => void; onOpenBatch: (batch: Batch) => void; onEditNote: (note: BatchNote) => void; onDeleteNote: (noteId: string) => void; onDeleteMortality: (record: MortalityRecord) => void; onEditExpense: (expense: Expense) => void; onDeleteExpense: (expense: Expense) => void }) {
  ...
  return <><section className="batches-toolbar"><p className="section-kicker">Grow-out operations</p><div className="batch-toolbar-actions">{selectedBatch && <button className="secondary-action" type="button" onClick={() => onEditBatch(selectedBatch)} title={`Edit ${selectedBatch.id}`}>Edit selected batch</button>}<button className="primary-action" type="button" onClick={onAddBatch}>Add livestock batch</button></div></section>
    ...
    {visibleBatch && <BatchTimeline batch={visibleBatch} expenses={expenses} batchNotes={batchNotes} mortalityRecords={mortalityRecords} onEditNote={onEditNote} onDeleteNote={onDeleteNote} onDeleteMortality={onDeleteMortality} onEditExpense={onEditExpense} onDeleteExpense={onDeleteExpense} />}
...
function BatchTimeline({ batch, expenses, batchNotes, mortalityRecords, onEditNote, onDeleteNote, onDeleteMortality, onEditExpense, onDeleteExpense }: { batch: Batch; expenses: Expense[]; batchNotes: BatchNote[]; mortalityRecords: MortalityRecord[]; onEditNote: (note: BatchNote) => void; onDeleteNote: (noteId: string) => void; onDeleteMortality: (record: MortalityRecord) => void; onEditExpense: (expense: Expense) => void; onDeleteExpense: (expense: Expense) => void }) {
  const purchaseNote: TimelineRecord = { id: 'purchase', date: batch.purchaseDate, type: 'Purchase', title: `${batch.headcount} ${batch.species.toLowerCase()} received`, detail: `Purchased from ${batch.supplier}`, source: 'system' }
  const expenseNotes: TimelineRecord[] = expenses.filter((expense) => expense.batchId === batch.id).map((expense) => ({ id: expense.id, date: expense.date, type: expense.category, title: `${expense.category} recorded`, detail: `${expense.description} · ${php.format(expense.amount)}`, source: 'expense' }))
  const manualNotes: TimelineRecord[] = batchNotes.filter((note) => note.batchId === batch.id).map((note) => ({ id: note.id, date: note.date, type: note.type, title: 'Farm note', detail: note.note, source: 'note' }))
  const lossNotes: TimelineRecord[] = mortalityRecords.filter((record) => record.batchId === batch.id).map((record) => ({ id: record.id, date: record.date, type: 'Mortality', title: `${record.headsLost} head${record.headsLost === 1 ? '' : 's'} lost`, detail: record.note || 'No reason recorded', source: 'mortality', headsLost: record.headsLost }))
  const timeline = [purchaseNote, ...expenseNotes, ...manualNotes, ...lossNotes].sort((first, second) => first.date.localeCompare(second.date))
  return <article className="performance-timeline batch-record-timeline"><div className="panel-heading"><div><p className="section-kicker">Selected batch record</p><h2>{batch.id} dates and notes</h2></div><span className="list-note">{timeline.length} recorded events</span></div><TimelineList batchId={batch.id} batchNotes={batchNotes} expenses={expenses} timeline={timeline} onEditNote={onEditNote} onDeleteNote={onDeleteNote} onDeleteMortality={onDeleteMortality} onEditExpense={onEditExpense} onDeleteExpense={onDeleteExpense} /></article>
}

function TimelineList({ batchId, batchNotes, expenses, timeline, onEditNote, onDeleteNote, onDeleteMortality, onEditPerformance, onDeletePerformance, onEditExpense, onDeleteExpense }: { batchId: string; batchNotes: BatchNote[]; expenses: Expense[]; timeline: TimelineRecord[]; onEditNote: (note: BatchNote) => void; onDeleteNote: (noteId: string) => void; onDeleteMortality: (record: MortalityRecord) => void; onEditPerformance?: (record: { batchId: string; sample: PerformanceSample }) => void; onDeletePerformance?: (batchId: string, sample: PerformanceSample) => void; onEditExpense: (expense: Expense) => void; onDeleteExpense: (expense: Expense) => void }) {
  return <div className="timeline-list">{timeline.map((note) => <div className="timeline-item" key={`${note.source}-${note.id}`}><time>{formatDate(note.date)}</time><span className={`timeline-marker ${note.type.toLowerCase().replaceAll(' ', '-')}`} aria-label={note.type === 'Mortality' ? 'Mortality event' : note.type === 'Medicine' ? 'Medicine event' : undefined}>{note.type === 'Mortality' ? '!' : note.type === 'Medicine' ? '+' : ''}</span><div><i>{note.type}</i><b>{note.title}</b><p>{note.detail}</p></div><div className="timeline-actions">{note.source === 'performance' ? <><button type="button" onClick={() => { if (onEditPerformance) onEditPerformance({ batchId, sample: { id: note.id, weekEnding: note.date, averageWeight: note.averageWeight ?? 0, feedBags: note.feedBags ?? 0, note: note.noteText ?? '' } }) }}>Edit record</button>{onDeletePerformance && <button className="danger-action" type="button" onClick={() => onDeletePerformance(batchId, { id: note.id, weekEnding: note.date, averageWeight: note.averageWeight ?? 0, feedBags: note.feedBags ?? 0, note: note.noteText ?? '' })}>Remove record</button>}</> : note.source === 'expense' ? <><button type="button" onClick={() => { const expense = expenses.find((item) => item.id === note.id); if (expense) onEditExpense(expense) }}>Edit expense</button><button className="danger-action" type="button" onClick={() => { const expense = expenses.find((item) => item.id === note.id); if (expense) onDeleteExpense(expense) }}>Remove expense</button></> : note.source === 'note' ? <><button type="button" onClick={() => { const noteRecord = batchNotes.find((item) => item.id === note.id); if (noteRecord) onEditNote(noteRecord) }}>Edit note</button><button className="danger-action" type="button" onClick={() => onDeleteNote(note.id)}>Remove note</button></> : note.source === 'mortality' ? <button className="danger-action" type="button" onClick={() => onDeleteMortality({ id: note.id, batchId, date: note.date, headsLost: note.headsLost ?? 0, note: note.detail === 'No reason recorded' ? '' : note.detail })}>Remove mortality</button> : <span>View only</span>}</div></div>)}</div>
}
...
function PerformancePanel({ batches, expenses, batchNotes, mortalityRecords, performanceByBatch, saleEstimates, onAddWeight, onAddNote, onSaveEstimate, onEditNote, onDeleteNote, onDeleteMortality, onEditPerformance, onDeletePerformance, onEditExpense, onDeleteExpense }: { batches: Batch[]; expenses: Expense[]; batchNotes: BatchNote[]; mortalityRecords: MortalityRecord[]; performanceByBatch: Record<string, PerformanceSample[]>; saleEstimates: Record<string, { price: number; weight: number }>; onAddWeight: () => void; onAddNote: () => void; onSaveEstimate: (batch: Batch, price: number, weight: number) => void; onEditNote: (note: BatchNote) => void; onDeleteNote: (noteId: string) => void; onDeleteMortality: (record: MortalityRecord) => void; onEditPerformance: (record: { batchId: string; sample: PerformanceSample }) => void; onDeletePerformance: (batchId: string, sample: PerformanceSample) => void; onEditExpense: (expense: Expense) => void; onDeleteExpense: (expense: Expense) => void }) {
  const activeBatches = batches.filter((batch) => batch.status === 'Active')
  ...
  const weeklyNotes: TimelineRecord[] = samples.map((sample, index) => ({ id: sample.id, date: chartDates[index], type: index === 0 ? 'Starting check' : 'Weekly check', title: `${sample.averageWeight} kg average weight`, detail: index === 0 ? 'Starting weight recorded on arrival' : `${sample.feedBags} of 50 kg feed bags used this week`, source: 'performance', averageWeight: sample.averageWeight, feedBags: sample.feedBags, noteText: sample.note }))
  const purchaseNote: TimelineRecord = { id: 'purchase', date: selectedBatch.purchaseDate, type: 'Purchase', title: `${selectedBatch.headcount} ${selectedBatch.species.toLowerCase()} received`, detail: `Purchased from ${selectedBatch.supplier}`, source: 'system' }
  const expenseNotes: TimelineRecord[] = expenses.filter((expense) => expense.batchId === selectedBatch.id).map((expense) => ({ id: expense.id, date: expense.date, type: expense.category, title: `${expense.category} recorded`, detail: `${expense.description} · ${php.format(expense.amount)}`, source: 'expense' }))
  const manualNotes: TimelineRecord[] = batchNotes.filter((note) => note.batchId === selectedBatch.id).map((note) => ({ id: note.id, date: note.date, type: note.type, title: 'Farm note', detail: note.note, source: 'note' }))
  const lossNotes: TimelineRecord[] = mortalityRecords.filter((record) => record.batchId === selectedBatch.id).map((record) => ({ id: record.id, date: record.date, type: 'Mortality', title: `${record.headsLost} head${record.headsLost === 1 ? '' : 's'} lost`, detail: record.note || 'No reason recorded', source: 'mortality', headsLost: record.headsLost }))
  ...
  return <section className="performance-panel">
    ...
    <article className="performance-timeline"><div className="panel-heading"><div><p className="section-kicker">Batch record</p><h2>{selectedDate ? `Dates and notes · ${formatDate(selectedDate)}` : 'Dates and notes'}</h2></div>{selectedDate ? <button className="quiet-action" type="button" onClick={() => setSelectedDayIndex(null)}>Show all notes</button> : <span className="list-note">{timeline.length} recorded events</span>}</div><TimelineList batchId={selectedBatch.id} batchNotes={batchNotes} expenses={expenses} timeline={visibleTimeline} onEditNote={onEditNote} onDeleteNote={onDeleteNote} onDeleteMortality={onDeleteMortality} onEditPerformance={onEditPerformance} onDeletePerformance={onDeletePerformance} onEditExpense={onEditExpense} onDeleteExpense={onDeleteExpense} /></article>
  </section>
}
