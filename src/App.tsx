...
function Dashboard({ batches, expenses, totalAnimals, totalCost, operationalCapital, onAddCapital, onEntry, onBatches, onOpenBatch }: { batches: Batch[]; expenses: Expense[]; totalAnimals: number; totalCost: number; operationalCapital: OperationalCapital; onAddCapital: () => void; onEntry: (kind: EntryKind) => void; onBatches: () => void; onOpenBatch: (batch: Batch) => void }) {
...
function CapitalForm({ onClose, onSubmit }: { onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) { return <div className="modal-backdrop" role="presentation"><section className="entry-modal" role="dialog" aria-modal="true" aria-labelledby="capital-title"><div><p className="section-kicker">Operational capital</p><h2 id="capital-title">Add capital</h2><p>Add funding available for livestock purchases, feed, and medicine only.</p></div><button className="close-modal" type="button" onClick={onClose} aria-label="Close capital form">Close</button><form onSubmit={onSubmit}><div className="form-grid"><label>Amount (PHP)<input name="amount" required min="0.01" step="0.01" type="number" autoFocus /></label><label>Date received<input name="receivedDate" required type="date" defaultValue="2026-09-07" /></label></div><label>Note<input name="description" required placeholder="Example: Additional grow-out capital" /></label><button className="submit-entry" type="submit">Add capital</button></form></section></div> }
